...
main_directory = r'C:\Users\Araz\Desktop\Flensburg\Big P\295\V5\Opt_20_295\Python Fully Automate run test\DLC1.2\DLC12'  # Update this path
...
wind_files.sort(key=lambda x: (int(x.split('_')[0][1:]), int(x.split('Seed')[1].split('.bts')[0])))

for wind_file in wind_files:
    # Load the current OPT-20-295-Monopile_InflowFile.dat file
    inflow_file_path = os.path.join(main_directory, 'OPT-20-295-Monopile_InflowFile.dat')
    ...
    # Modify the OPT-20-295-Monopile.fst file to dynamically update HydroDyn reference
    fst_file_path = os.path.join(main_directory, 'OPT-20-295-Monopile.fst')
    ...
    start_openfast_batch = os.path.join(main_directory, 'start_OpenFAST_v3-41.bat')
    ...
    output_files = ['OPT-20-295-Monopile.out', 'OPT-20-295-Monopile.outb']
    new_output_name = f'OPT-MP-V{next_wind_speed}_S{next_seed}'
